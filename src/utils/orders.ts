...
export async function getOrders(): Promise<OrderPayload[]> {
  try {
    ...
    querySnapshot.forEach((doc) => {
      const data = doc.data() as OrderPayload;
      if (data) {
        list.push({
          ...data,
          id: data.id || doc.id
...
export async function saveOrder(order: OrderPayload): Promise<void> {
  try {
    ...
    // Tự động đồng bộ hóa đơn hàng trực tiếp lên Google Sheets từ phía client để đảm bảo thời gian thực
    let sheetsUrl = "";
    try {
      const settingsSnap = await getDoc(doc(db, "gmail", "settings"));
      if (settingsSnap.exists()) {
        const configData = settingsSnap.data();
        sheetsUrl = configData.googleSheetsUrl || configData.googleSheetUrl || "";
      }
    } catch (err) {
      console.warn("[Client Order Sync] Failed to fetch googleSheetsUrl from Firestore:", err);
    }
    if (!sheetsUrl) {
      sheetsUrl = localStorage.getItem('yeng_google_sheets_url') || "";
    }

    if (sheetsUrl) {
      try {
        console.log("[Client Order Sync] Đang tự động đẩy đơn sang Google Sheets:", sheetsUrl);
        
        const itemsFormatted = (finalCleanedOrder.items ?? []).map((item: any) => 
          `${item.product?.name || 'Sản phẩm'} (Phân loại: ${item.version || '—'}) x${item.quantity}`
        ).join(", ");

        const totalQty = (finalCleanedOrder.items ?? []).reduce((sum: number, item: any) => sum + item.quantity, 0);
        const paymentMethod = finalCleanedOrder.payment?.method || '';
        ...
        const subtotalVal = finalCleanedOrder.subtotal ?? 0;
        const calculatedPaid = finalCleanedOrder.paidAmount !== undefined ? finalCleanedOrder.paidAmount : (isHalfDeposit ? Math.round(subtotalVal * 0.5) : subtotalVal);

        const payload = {
          timestamp: new Date(finalCleanedOrder.timestamp).toLocaleString('vi-VN'),
          email: finalCleanedOrder.contact?.email ?? "",
          snsLink: finalCleanedOrder.contact?.snsLink,
          quantity: totalQty,
          invoiceImage: finalCleanedOrder.payment?.invoiceImage || "",
          customerName: finalCleanedOrder.shipping?.receiverName || '',
          phone: finalCleanedOrder.shipping?.phone || "",
          address: finalCleanedOrder.shipping?.address || "",
          shippingMethod: finalCleanedOrder.shipping?.method ?? "",
          note: finalCleanedOrder.note && finalCleanedOrder.note !== "Không có" ? `[Sản phẩm: ${itemsFormatted}] | ${finalCleanedOrder.note}` : itemsFormatted,
          paidAmount: calculatedPaid,
          totalAmount: subtotalVal,
          items: (finalCleanedOrder.items ?? []).map((item: any) => ({
            name: item.product?.name || 'Sản phẩm',
            version: item.version || 'Mặc định',
            quantity: item.quantity || 1
          })),
          cartItems: (finalCleanedOrder.items ?? []).map((item: any) => ({
            productName: item.product?.name || 'Sản phẩm',
            version: item.version || 'Mặc định',
            quantity: item.quantity || 1
          })),
          productName: finalCleanedOrder.items?.[0]?.product?.name || "",
          version: finalCleanedOrder.items?.[0]?.version || "",
          orderId: finalCleanedOrder.id,
          products: itemsFormatted,
          paymentMethod: finalCleanedOrder.payment?.method || "Chưa xác định"
        };
        ...
        console.log("[Client Order Sync] Đã gửi webhook sang Google Sheets thành công.");
        
        // Cập nhật trạng thái googleSheetsSynced thành true trong database
        finalCleanedOrder.googleSheetsSynced = true;
        await setDoc(docRef, { googleSheetsSynced: true }, { merge: true });
        console.log("[Client Order Sync] Đã cập nhật trạng thái googleSheetsSynced thành true trên Firestore.");
      } catch (err: any) {
        console.error("[Client Order Sync] Lỗi khi tự động đồng bộ Google Sheets:", err.message);
      }
    }
